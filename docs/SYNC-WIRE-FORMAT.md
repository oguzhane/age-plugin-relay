...
| `tag` | string | `base64(SHA-256(unwrap_recipient)[:16])`. Routing tag |
...
The relay-server responds in one of two formats. The server decides the format: JSON by default, or SSE when started with the `-stream` flag. The plugin auto-detects via the `Content-Type` header.

### 2a. JSON response (default)
...
### 2b. SSE response (server started with `-stream`)
...
SSE is for long-running relay scenarios (approval flows, remote YubiKey touch). Enabled on the server with `-stream`. The connection stays open through proxies and load balancers.
...
7. Recompute `SHA-256("1.unwrap." + intent_id + "." + tag + "." + expires_at + "." + intent_claim_pub)`.
...
|---|---|---|
| Request (plugin → server) | `SHA-256("{version}.{action}.{intent_id}.{tag}.{expires_at}.{intent_claim_pub}")` | `SHA-256("1.unwrap.a3f1...7b8c.QPg24ggKk7xKd2t3c5rL9A.1715350800.AAAA...")` |
